...
from Alch.columns import Column
from Alch.db import Database
from Alch.fields import IntegerField, TextField, RealField
from Alch.models import Model
from Alch.utils import try_or_error

logging.basicConfig(filename="alhemius.log", level=logging.INFO)
...
def main():
    ...
    categories = [['Apple', 'Apple mobile phones'],
    ...
    Item.session(db).update(title='Nokia 3310').filter_by(
        title='iPhone 7').all()
    item = Item.session(db).query(['items.id', 'items.title', 'items.price', 'categories.title',
                                   'producers.title']).foreign_key().all()
    logging.info(item.serialize_json())  # Log serialize to json
