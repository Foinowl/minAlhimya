...
from .utils import OrderedClass, try_or_error
...
class Model(metaclass=OrderedClass):
    ...
    def __call__(self, *args, **kwargs):
        print('Model', args, kwargs, self.__request_constructor__)
    ...
    def create_table(cls):
        ...
        request_fk = ', '.join(
            ['FOREIGN KEY(`{}`) REFERENCES {} ON UPDATE CASCADE ON DELETE CASCADE'.format(col[0], row[1]) for col in
             cls.columns() for row in col[1] if row[0] == 'foreign_key'])
        print('request_fk', request_fk)
